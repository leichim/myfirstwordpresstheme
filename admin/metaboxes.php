...
function set_get_metaboxes() {
    ...
    if (isset($post_id)) { $template_file = get_post_meta($post_id,'_wp_page_template',TRUE); } // check for a template type
    ...
    $meta_box['portfolio_project'] = array(
        ...
        'fields' => array(
            ...
            array(
                      "id" => "project_featured",
                      "std" => "",
                      "name" => __("Feature project:","msign"),
                      "desc" => __("Put project on top of the portfolio page with its slider. Visually not recommended if using ajax to load previews.","msign"),
                      "type" => "checkbox",
            ),							
            array(
                "id" => "post_layout",
            ...
            array(
                "id" => "post_related_title",
                "std" => "",
                "name" => __("Title above related projects:","msign"),
                "desc" => __("Use this if you want to override the global theme settings for related projects title for this post only.","msign"),
                "type" => "text",
            ),
            array(
                "id" => "post_related_number",
                "std" => "",
                "name" => __("Number of related projects to show:","msign"),
                "desc" => __("Use this if you want to override the global theme settings for related projects number for this project only.","msign"),
                "type" => "text",
            ),					
            array(
                "id" => "post_related",
                "std" => "",
                "name" => __("Disable related posts for this project:","msign"),
                "desc" => __("Use this if you want to override the global theme settings for the related posts for this project only .","msign"),
                "type" => "checkbox",
            ),					
            array(
                "id" => "post_seo_title",
    ...
    if (isset($template_file) && $template_file == 'contact.php' ) {
        $meta_box['page'] = array(
            'id' => 'post-format-meta',  
...
add_action('add_meta_boxes', 'msign_add_box');
  ...
  foreach ($meta_box[$post->post_type]['fields'] as $field) {
      ...
      switch ($field['type']) {
		  ...
		  case 'layout':
		  	 echo '<div class="boxed-fields">';
			 foreach ($field['options'] as $option) {
              	echo '<input type="radio" class="lay-out ' . $option . '" id="'. $option .'" name="' . $field['id'] . '" value="' . $option . '"' . ( $meta == $option  ? ' checked="checked"' : '' ) . ' /><label for="'. $option .'">'. $option .'</label>'; 
			 }
			 echo '<br /><span class="description">'. $field['desc'] . '</span></div>';
             break;	
		  case 'postloop':
...
function msign_save_data($post_id) {
    ...
    $nonce = $_POST['msign_meta_box_nonce'];
