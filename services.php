 ...
 * This template gives an overview of services
 */
get_header(); ?>
        ...
        <main id="content" role="main" itemscope itemtype="http://schema.org/ProfessionalService">
            ...
            <?php $post_amount = get_post_meta($post->ID, 'posts_amount', true);
                ...
                if( $post_order == 'title' || $post_order == 'author'  ) {
                    $args = array('orderby' => $post_order, 'order' => 'ASC', 'paged' => $paged, 'posts_per_page' => $post_amount, 'post_type' => 'services'); 
                } else {
                    $args = array( 'orderby' => $post_order, 'paged' => $paged, 'posts_per_page' => $post_amount, 'post_type' => 'services'); 
                }
